...
*Example Request*
When the phonebook is empty, no content is returned. This is indicated with a http status https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/204[204 - No Content].

include::{snippets}/get-all-contacts-when-phonebook-is-empty/http-response.adoc[]

When there are items in the phonebook, the response contains an array of Contacts.
